...
namespace othopedics
{
	 ...
	 * \return true,if intersection
	 */
	inline bool lineIntersect3d_2(const Eigen::Vector3d& p1, const Eigen::Vector3d& e1, const Eigen::Vector3d& p2, const Eigen::Vector3d& e2, Eigen::Vector3d& intersectP)
	{
		Eigen::Vector3d v1 = e1 - p1;
		Eigen::Vector3d v2 = e2 - p2;
		if (v1.dot(v2) == 1)
		{
			// 两线平行
			return false;
		...
		// if (qFuzzyIsNull(vecS1.lengthSquared())) {        　　　　　　return false;    　　　　　　}　
		// float num2 = vecS2.dot(vecS1) / vecS1.norm();
		// if (num2 > 1 || num < 0) {
		// 	return false;//num2的大小还可以判断是延长线相交还是线段相交
		// }
		float num2 = vecS2.dot(vecS1) / vecS1.squaredNorm();
