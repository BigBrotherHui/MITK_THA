...
void Femur::InitFemurLocalFrame()
{
		...
		&& GetLandMark(ELandMarks::f_DFCA, DFCA) && GetLandMark(ELandMarks::f_PFCA, PFCA))
	{
		...
		Eigen::Vector3d p1, e1, p2, e2;
		p1 << 1, 1, 0;
		p2 << 2, 0, 0;
		e1 << 1, 0, 0;
		e2 << 0, 1, 0;
		auto b = lineIntersect3d_2(p1, e1, p2, e2, intersection);
		MITK_WARN <<b <<"," << intersection;

		if (!lineIntersect3d_2(Eigen::Vector3d(fhc_projected), Eigen::Vector3d(fnc_projected), PFCA, DFCA, intersection))
		{
			MITK_WARN << "line not intersection";
			return;
		}
		SetLandMark(ELandMarks::f_NeckCanalIntersectPoint, intersection.data());
